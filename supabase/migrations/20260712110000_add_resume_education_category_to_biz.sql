-- 이력서 학력 분류를 기업 지원자 조회 결과에도 포함합니다.

create or replace function public.get_applicants_by_company_code(company_code text)
returns table (
  id uuid, company_id uuid, name text, role text, experience text,
  status public.applicant_status, submitted_at timestamptz, email text, phone text,
  location text, headline text, photo_path text, education text, educations jsonb,
  recent_job text, experiences jsonb, skills text[], tools text[], resume_url text,
  portfolio jsonb, duration text, simulation jsonb, created_at timestamptz,
  updated_at timestamptz, desired_salary text, preferred_region text,
  employment_type text, resume_title text, resume_source_type text
)
language sql
stable
security definer
...
as $get_applicants$
  select
    ...
    case
      ...
      when exp.total_months > 0 and exp.total_months >= 12 then (exp.total_months / 12)::int::text || '년'
      when exp.total_months > 0 then exp.total_months::int::text || '개월'
    ...
    coalesce(nullif(r.basics->>'location', ''), nullif(array_to_string(seeker.work_regions, ', '), ''), '거주 지역 미입력') as location,
    ...
    coalesce(nullif(concat_ws(' · ', nullif(r.experiences->0->>'company', ''), nullif(r.experiences->0->>'role', ''), nullif(r.experiences->0->>'period', '')), ''), '경력 미입력') as recent_job,
    ...
    jsonb_build_array(jsonb_build_object('step', 1, 'title', js.title, 'answer', coalesce(s.response_text, '답변 없음'))) as simulation,
    s.created_at,
    ...
    coalesce(nullif(r.job_conditions->>'preferred_region', ''), nullif(array_to_string(seeker.work_regions, ', '), ''), '희망 지역 미입력') as preferred_region,
    coalesce(nullif(r.job_conditions->>'employment_type', ''), nullif(array_to_string(seeker.employment_types, ', '), ''), '근무 형태 미입력') as employment_type,
  ...
  left join lateral (
    select resume.* from public.resumes resume
  ...
  left join lateral (
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'category', coalesce(item.value->>'category', ''),
          ...
          'description', coalesce(
            ...
            nullif(concat_ws(' ', nullif(item.value->>'category', ''), nullif(item.value->>'school', ''), nullif(item.value->>'major', ''), nullif(item.value->>'status', '')), ''),
            ''
          )
        ) order by item.ord
      ) filter (
        where concat_ws('', item.value->>'category', item.value->>'school', item.value->>'major', item.value->>'status', item.value->>'description') <> ''
      ),
      '[]'::jsonb
    ) as items
  ...
  left join lateral (
    select
      coalesce(
        ...
        ) filter (where coalesce(item.value->>'company', item.value->>'role', item.value->>'period', item.value->>'description', '') <> ''),
        '[]'::jsonb
      ) as items,
      coalesce(sum(case when coalesce(item.value->>'durationMonths', '') ~ '^[0-9]+$' then (item.value->>'durationMonths')::int else 0 end), 0) as total_months
  ...
  left join lateral (
    select coalesce(
      ...
      ) filter (where coalesce(item.value->>'title', item.value->>'description', item.value->>'url', '') <> ''),
      '[]'::jsonb
    ) as items
